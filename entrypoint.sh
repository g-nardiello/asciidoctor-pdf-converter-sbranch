...
git checkout "$GITHUB_SHA" -B $INPUT_WORKING_BRANCH
...
if [[ $INPUT_WORKING_BRANCH != ${GITHUB_REF##*/} ]]; then
  echo "Removing AsciiDoc and working files from $INPUT_WORKING_BRANCH branch as different from main branch ${GITHUB_REF##*/}"
...
echo "Adding output files to $INPUT_WORKING_BRANCH branch."
...
git push -f --set-upstream origin $INPUT_WORKING_BRANCH
